...
/* note:
   ...
   - DMA_MAXSIZE: if the transacion Size > DMA_MAXSIZE -> multiple DMA transactions (because DMA transaction size
   register is 16bit) */
...
#define LCD_SPI_RXFIFOCLEAR(hlcdspi, dummy)                                                                            \
    while (hlcdspi.Instance->SR & SPI_SR_RXNE)                                                                         \
    dummy = hlcdspi.Instance->DR
...
#define LCD_SPI_RXFIFOCLEAR(hlcdspi, dummy)                                                                            \
    while (hlcdspi.Instance->SR & SPI_SR_RXP)                                                                          \
    dummy = hlcdspi.Instance->RXDR
...
#endif
    LL_GPIO_SetPinMode(SPI_SCK_GPIO_Port, SPI_SCK_Pin, LL_GPIO_MODE_OUTPUT); /* GPIO mode = output */
    while (DummySize--) { /* Dummy pulses */
        HAL_GPIO_WritePin(SPI_SCK_GPIO_Port, SPI_SCK_Pin, 1 - LCDTS_SPI_DEFSTATE);
        HAL_GPIO_WritePin(SPI_SCK_GPIO_Port, SPI_SCK_Pin, LCDTS_SPI_DEFSTATE);
    }
    LL_GPIO_SetPinMode(SPI_SCK_GPIO_Port, SPI_SCK_Pin, LL_GPIO_MODE_ALTERNATE); /* GPIO mode = alternative */
...
uint32_t LCD_IO_DmaBusy(void) { return 0; }
...
#define LcdSemNew0                                                                                                     \
    LcdSemIdHandle = osSemaphoreCreate(osSemaphore(LcdSemId), 1);                                                      \
    osSemaphoreWait(LcdSemIdHandle, 0)
...
__weak void LCD_IO_DmaTxCpltCallback(SPI_HandleTypeDef* hspi) { UNUSED(hspi); }
...
#endif
{
    if (hspi == &LCDTS_SPI_HANDLE) {
        if (dmastatus.size > dmastatus.trsize) { /* dma operation is still required */
        ...
        else { /* dma operations have ended */
...
#if LCD_DMA_TX == 1
    if ((Size > DMA_MINSIZE) && (!LCD_DMA_UNABLE((uint32_t)pData))) { /* DMA mode */
        if (Mode & LCD_IO_DATA8) { /* 8bit DMA */
...
//-----------------------------------------------------------------------------
/* DMA operation end callback function prototype */
__weak void LCD_IO_DmaRxCpltCallback(SPI_HandleTypeDef* hspi) { UNUSED(hspi); }
...
#if LCD_DMA_TX == 1
    HAL_SPI_RegisterCallback(&LCD_SPI_HANDLE, HAL_SPI_TX_COMPLETE_CB_ID,
                             (pSPI_CallbackTypeDef)HAL_SPI_TxCpltCallback_Lcd);
#endif
#if LCD_DMA_RX == 1
    HAL_SPI_RegisterCallback(&LCD_SPI_HANDLE, HAL_SPI_TX_COMPLETE_CB_ID,
                             (pSPI_CallbackTypeDef)HAL_SPI_RxCpltCallback_Lcd);
#endif
...
void LCD_IO_Transaction(uint16_t Cmd, uint8_t* pData, uint32_t Size, uint32_t DummySize, uint32_t Mode) {
    ...
    if (Size == 0) { /* only command byte or word */
        HAL_GPIO_WritePin(LCD_CS_GPIO_Port, LCD_CS_Pin, GPIO_PIN_SET);
        LcdTransEnd();
        return;
    }
    ...
    /* Datas write or read */
    if (Mode & LCD_IO_WRITE) { /* Write Lcd */
...
#if LCD_SPI_MODE != 0
    else if (Mode & LCD_IO_READ) { /* Read LCD */
...
TS_DrvTypeDef xpt2046_ts_drv = {xpt2046_ts_Init, 0, 0, 0, xpt2046_ts_DetectTouch, xpt2046_ts_GetXY, 0, 0, 0, 0};
...
//-----------------------------------------------------------------------------
void xpt2046_ts_GetXY(uint16_t DeviceAddr, uint16_t* X, uint16_t* Y) { *X = tx, *Y = ty; }
